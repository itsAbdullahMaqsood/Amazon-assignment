...
const PrimeNav = () => {
    ...
    return (
        <div className="flex items-center gap-2 px-6 py-3">
            <Link href="/prime-video" className="mr-4 text-lg lowercase tracking-tight">
                <span className="font-semibold">prime</span> video
            </Link>

            <nav aria-label="Prime Video sections" className="flex items-center gap-1">
                {tabs.map((tab) => (
                    <button
                        key={tab}
                        onClick={() => setActive(tab)}
                        aria-current={active === tab ? "page" : undefined}
                        className={`px-4 py-2 rounded font-semibold text-lg cursor-pointer ${
